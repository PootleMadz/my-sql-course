...
*/
SELECT COUNT(*) as AllCount FROM Message; -- number of rows in table
SELECT COUNT(*) as GreenCount FROM Message m WHERE m.Colour = 'Green'; -- number of green messages
(SELECT COUNT(*) AS NotGreenCount)
SELECT COUNT(*) as NotGreenCount FROM Message m WHERE m.Colour <> 'Green'; -- those that aren't green but won't include the nulls
 
 -- another method to bring back anything that isn't green from messages
 SELECT
    t.AllCount
    ,t.GreenCount
