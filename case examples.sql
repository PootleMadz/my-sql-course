...
ORDER BY ps.AdmittedDate, ps.PatientId;
...
FROM
	dbo.PatientStay ps
ORDER BY PatientTariffGroup
	, ps.Tariff
	, ps.PatientId

-- Optional advanced question: how many patients are in each PatientTariffGroup
SELECT 
    p.PatientTariffGroup -- New Column - to add the tariff group 
    , COUNT(*) AS NumberOfPatients -- New count of how many are in each tariff - called number of patients
FROM ( --Start of Tabled Sub-query
SELECT
	ps.PatientId
	, ps.AdmittedDate
	, ps.Tariff
